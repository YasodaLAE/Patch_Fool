...
def denormalize_tensor(tensor, mu, std):
    """Reverses the normalization on a tensor batch."""
    # Convert mu and std lists to tensors for broadcastable operation
    mu_tensor = torch.tensor(mu, device=tensor.device).view(-1, 1, 1)
    std_tensor = torch.tensor(std, device=tensor.device).view(-1, 1, 1)

    # De-normalize: result = (tensor * std) + mu
    result = tensor.clone() * std_tensor
    result = result + mu_tensor

    # Clamp to [0, 1] range which is expected for ToPILImage
...
def visualize_attention_map(args, atten1, atten2, image1, image2, original_result, after_attack_result, max_patch_index):
    ...
    mu_tensor = torch.tensor(args.mu).view(3, 1, 1).to(image1.device)
    std_tensor = torch.tensor(args.std).view(3, 1, 1).to(image1.device)

    # Apply de-normalization
    clean_image1 = denormalize_tensor(image1, args.mu, args.std)
    clean_image2 = denormalize_tensor(image2, args.mu, args.std)
    ...
    for i in range(pic_num):
        ...
        original_img = to_PIL(clean_image1[i].squeeze())
        after_attack_img = to_PIL(clean_image2[i].squeeze())
        original_atten = [x[i] for x in atten1]  # [197x197]
        after_attack_atten = [x[i] for x in atten2]
        
        patch_index = max_patch_index[i].cpu().numpy().flatten()
        
        # We only draw the first patch if multiple are used (args.num_patch > 1)
        index = patch_index[0] 
        
        # Calculate row and column indices for the patch
        # patch_per_line is the number of patches across the image (e.g., 14 for 224x224/16)
        # patch_size is the pixel size of one patch (e.g., 16)
        row_idx = (index // patch_per_line) 
        col_idx = (index % patch_per_line)

        # Calculate pixel coordinates for the top-left and bottom-right corners
        top_left_x = col_idx * patch_size
        top_left_y = row_idx * patch_size
        ...
        patch_box = [(top_left_x, top_left_y), (bottom_right_x, bottom_right_y)]
        
        # Draw the rectangle on the ADVERSARIAL image
        draw = ImageDraw.Draw(after_attack_img)
        draw.rectangle(patch_box, outline="red", width=3) # Draw a thick red box
        
        # Draw the rectangle on the ORIGINAL image (for comparison)
        draw = ImageDraw.Draw(original_img)
        draw.rectangle(patch_box, outline="blue", width=3) # Draw a different color box (blue)

        # --- END OF PATCH HIGHLIGHTING CODE ---
