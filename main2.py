        ...
        for train_iter_num in range(args.train_attack_iters):
            ...
            if args.attack_mode == 'Attention':
                ...
                for atten_num in range_list:
                    ...
                    if 'DeiT' in args.network:
                        device = atten_map.device
                        target_tensor = (max_patch_index_matrix + 1).to(device)
                        atten_loss = F.nll_loss(atten_map, target_tensor)
                    else:
                        device = atten_map.device
                        target_tensor = (max_patch_index_matrix).to(device)
                        atten_loss = F.nll_loss(atten_map, target_tensor)
